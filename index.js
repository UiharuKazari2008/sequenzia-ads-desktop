...
const cliArgs = yargs(hideBin(process.argv))
    ...
    .option('generateHEIC', {
        alias: 'g',
        type: 'boolean',
        description: 'Generate Dynamic Wallpapers'
    })
...
async function generateDynamicWallpapers(pair) {
    const light = fs.readdirSync(pair.light).filter(file => file.endsWith('.png'));
    ...
    if (pair.light_start && pair.dark_start) {
        ...
        const nightInterval = Math.floor(nightPeriod / dark.length);
        let currentTime = moment(timeRanges[0], 'HH:mm');
        ...
        for (const i in dark) {
            ...
            dypair.push(obj);
            currentTime.add(nightInterval, 'minutes');
        }

        fs.writeFileSync(path.join(pair.path, `${filename}.json`), JSON.stringify(dypair));
        await new Promise(ok => {
            const genImage = spawn(((config.wallpapper_exec) ? config.wallpapper_exec : "wallpapper"), [
                '-i', path.join(pair.path, `${filename}.json`),
                '-o', path.join(pair.path, `${filename}.heic`)], {encoding: 'utf8'});
